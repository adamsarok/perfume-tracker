...
export async function getWornBeforeID(cursor: Date | null, pageSize: number) : Promise<PerfumeWornDTO[]> {
    const qry = `/perfume-events/worn-perfumes?cursor=${encodeURIComponent(cursor ? cursor.toISOString() : "")}&pageSize=${encodeURIComponent(pageSize)}`;
    const r2ApiUrl = await getR2ApiUrl();
    const response = (await get<PerfumeWornDTO[]>(qry)).data;
    response.forEach(x => x.perfumeImageUrl = getImageUrl(x.perfumeImageObjectKey, r2ApiUrl));
    return response;
}
