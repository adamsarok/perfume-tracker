...
DECLARE connstr CONSTANT varchar(255) := 'dbname=perfumetracker host=IPADDRESS port=5432 user=postgres password=PASSWORD';
...
BEGIN
    ...
    SELECT * FROM dblink(connstr, 
        'SELECT id, house, perfume, rating, notes, ml, "imageObjectKey", autumn, spring, summer, winter, "Created_At", "Updated_At" FROM public."Perfume"')
    ...
    SELECT * FROM dblink(connstr,
                        'SELECT "id", "perfumeId", "suggestedOn" FROM public."PerfumeSuggested"')
    ...
    SELECT * FROM dblink(connstr,
                        'SELECT "id", "tag", "color", "Created_At", "Updated_At" FROM public."Tag"')
    ...
    SELECT * FROM dblink(connstr,
                        'SELECT "id", "perfumeId", "tagId", "Created_At" FROM public."PerfumeTag"')
    ...
    SELECT * FROM dblink(connstr,
                        'SELECT "id", "perfumeId", "wornOn" FROM public."PerfumeWorn"')
    ...
    SELECT * FROM dblink(connstr,
                        'SELECT "id", "query", "recommendations", date FROM public."Recommendation"')
