...
public interface ICreateUser {
	Task Create(string? userName, string? password, string? role, string? email, bool isEmailConfirmed = false);
}
public class CreateUser(ILogger<CreateUser> logger, UserManager<PerfumeIdentityUser> userManager, PerfumeTrackerContext context) : ICreateUser {
	public async Task Create(string? userName, string? password, string? role, string? email, bool isEmailConfirmed = false) {
