...
public class PerfumeRepo(PerfumetrackerContext context, SettingsRepo settingsRepo, IMediator mediator) {
	public record PerfumeAddedEvent(Perfume perfume) : INotification;
	public record PerfumeUpdatedEvent(Perfume perfume) : INotification;
	//public record PerfumeAddedEvent(Perfume perfume) : INotification;

	public async Task<List<PerfumeWithWornStatsDto>> GetPerfumesWithWorn(string? fulltext = null) {
		var settings = await settingsRepo.GetSettingsOrDefault(userId); //TODO implement when multi user is needed
